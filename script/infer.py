'''
file
author        feihong (feihong2@zhumanggroup.com)
Date          2023-08-24 19:03:31
version       1.0
...
LastEditTime  2024-03-28 11:37:45
FilePath      /test_dataset/script/infer.py
brief
copyright     Copyright (c) 2023 by feihong (feihong2@zhumanggroup.com), All Rights Reserved.
'''
...
def main(args):
    if args.image_path is None and args.image_filelist is None and args.image_root_path is None:
        raise ValueError("`--image_path` or `--image_filelist` and `--image_root_path` should be set.")

    if args.image_filelist is None or args.image_root_path is None:
        raise ValueError("`--image_filelist` and `--image_root_path` should be set at the same time.")

    if args.image_path is not None :
        imgs_list, _ = get_image_list(args.image_path)
    elif args.image_root_path is not None and args.image_filelist is not None:
        imgs_list, _ = get_image_filelist(args.image_root_path, args.image_filelist)
